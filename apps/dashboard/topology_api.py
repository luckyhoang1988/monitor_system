...
def _latest_ap_snapshot(ac: Device) -> dict[str, dict]:
    """MAC normalized → {name, ip, online, client_count, run_state}."""
    latest_ts = (
...
def build_topology_graph(
    ...
    """Trả {nodes, edges, meta} cho Cytoscape."""
    if ac is None:
        ac = (
    ...
    if switch_filter:
        ...
        switch_ids.update(links_qs.values_list("local_device_id", flat=True).distinct())
    ...
    for link in links_qs.order_by("local_device__name", "local_port"):
        ...
        edges.append({
            "data": {
                "id": f"e-{sw_nid}-{ap_nid}",
                "source": sw_nid,
                "target": ap_nid,
                "label": link.local_port,
                "confirmed": link.is_confirmed,
            },
        })

    # AP trên AC chưa có link LLDP → orphan
    orphan_count = 0
