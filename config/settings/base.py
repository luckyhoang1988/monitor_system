...
# Số mẫu tối đa giữ trong mỗi ring-buffer (~1000 mẫu ≈ 25h @90s → phủ chart raw-tier 24h).
METRICS_SERIES_MAX_SAMPLES = env.int("METRICS_SERIES_MAX_SAMPLES", default=1000)
...
# worker khi chu kỳ poll ngắn (90s). 1 walk chết ≈ timeout×(retries+1).
SNMP_TIMEOUT_SECS = env.int("SNMP_TIMEOUT_SECS", default=5)
SNMP_RETRIES      = env.int("SNMP_RETRIES", default=1)

# Alert engine
ALERT_GRACE_PERIOD_SECS    = 135  # min seconds before "no data" is treated as offline (1.5× poll interval)
