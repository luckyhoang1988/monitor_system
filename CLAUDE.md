...
## Tech Stack
Django 5.x + Bootstrap 5 · PostgreSQL (prod) / SQLite (dev) · Celery + Redis + django-celery-beat · Netmiko (cisco_ios/huawei_vrp) · pysnmp + easysnmp · pywinrm + PowerShell · Chart.js (AJAX) · Alert: Email SMTP + Telegram.

## Luồng dữ liệu
```
Celery Beat (5') → CollectorFactory → SNMP/SSH/WinRM
...
- Docker Compose: `app` (gunicorn) + `worker` + `beat` + `db` (postgres16) + `redis` + `nginx`. Code **build vào image** (`build: .`, không bind-mount).
